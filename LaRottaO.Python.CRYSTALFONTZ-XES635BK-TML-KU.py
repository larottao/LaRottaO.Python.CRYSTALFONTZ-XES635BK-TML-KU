...
# by Luis La Rotta
# Port of the great C# code made by  Robert M. Meffe in February 2009
...
# Global variables
last_serial_received_values = []  # List to store the last two received values

def __init__(self):    
        
        # Send initialization commands for CRYSTALFONTZ XES635BK-TML-KU
        send_raw_command([0x01, 0x00, 0x9F, 0x16])
        send_raw_command([0x1E, 0x00, 0xC6, 0x00])     

def write_line(line_number, text, xpos, padded):
...
def send_command(command, data):
        ...
        # Print the hex values to the console
        print("Sending:", ' '.join(format(x, '02X') for x in send_list))

        _sp.write(bytearray(send_list))

def send_raw_command(data):
        # Print the hex values to the console
        print("Sending Raw Command:", ' '.join(format(x, '02X') for x in data))

        _sp.write(bytearray(data))
...
def button_interpretation(hex_data):  
    ...
    # Check the conditions for button presses based on the last two messages
    ...
    while True:
        try:
            if _sp.in_waiting > 0:
                ...
                #print(f"[{current_time}] Received data (hex): {hex_data}")                 
...
try:
    write_line(0, "PEQLL", 0, True)
    write_line(1, "PEQLL", 0, True)
    write_line(2, "PEQLL", 0, True)
    write_line(3, "PEQLL", 0, True)
